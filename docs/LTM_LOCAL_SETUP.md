...
1. Edit the CSV: `include=Y`, `department_code` (`FIN|PROC|OP|HR|IT|BD|SAL|CEO`), optional `manager_entra_id`. Entra `department` is empty for most users — set `department_code` manually. Users in `CEO` may assign to anyone whose `manager_entra_id` is their Entra id (see `assignment_policy.json`).
